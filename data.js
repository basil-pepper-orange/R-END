...
const CHARACTERS_ALL = [
  ...
  { id: 'alvis', elem: 'sea', name: 'アルヴィス', title: '白鎧の機械兵', rarity: 3,
    ...
    lost: { name: '強撃の盾', type: 'oath', power: 15.0, atkDown: 0.50, turns: 3,
            desc: '守るためだけに持っていた盾を、はじめて振り抜く。攻撃力の15倍のダメージ。3ターンのあいだ敵の攻撃力を50%下げる。',
  ...
  { id: 'owen', elem: 'forest', name: 'オーウェン', title: '最後の司書', rarity: 3,
    ...
    lost: { name: '書の魔獣', type: 'oath', power: 20.0, refillAll: 99,
            desc: '二冊とない召喚書を破り、獣を呼ぶ。攻撃力の20倍のダメージ。味方全員のスキル使用回数が全快する。',
  ...
  { id: 'raizu', elem: 'sea', name: 'ライズ', title: '一擲の賭博師', rarity: 4,
    ...
    own: { name: '賭ける', type: 'bet', rate: 0.60, power: 5.0, step: 0.5, cap: 10, hurt: 0.12,
    ...
    lost: { name: '大勝負', type: 'oath', gamble: 0.60, power: 38.0, silence: 5, healSelf: 1.00,
            desc: '指輪も有り金も卓に置く。60%で「攻撃力の38倍のダメージ＋5ターン敵の固有技を封じる＋自分のHPが全回復」。外れると何も起きない。',
...
const ENEMIES_ALL = [
...

  {
    ...
    desc: '崩壊のとき、ただひとり何ひとつ忘れようとしなかった王。臣も城も灰に還ったのに、戴冠だけが終わらない。覚えているという一点で、重さをもって迫ってくる。',
...

  {
    ...
    ending: true,
    story: '灰の降る速さが、人が生きる速さに戻った。<br>過ぎてしまった年月は返ってこない。<br>ただ、この先忘れるまでには、<br>懐かしく手触りのある時間がかかるだろう。',
    desc: '世界が忘れるまでの記憶の時間軸を早めていたもの。花の名も、街の名も、隣にいた人の顔も、覚えておくより先に薄れていく——世界が灰になったのは、これが降りつづけていたからである。抗うにはあまりにも大きすぎる存在だったが、彼らはたどり着いた。この相手とだけは、HPと同時に「過ぎていく年月」とも戦うことになる。',
...
const ENEMY_LINES = {
  ...
  e1: {
    hurt: ['……痛い。まだ痛むのか', 'やめろ、返してくれ', '何を、奪われている'],
    kill: ['眠った。……静かだ', '動かない。もう、動かない', 'ひとり、減った'],
    low:  ['消える。俺が、消える', 'まだ痛いのに', '俺は、どこにいる'],
    win:  ['……帰るな', 'ここに、いてくれ', 'ひとりは、嫌だ'],
    lose: ['花が……そうか、花か', '……母、さん', '俺の、名前は'],
    turn: ['……誰だ', '返してくれ', '角が、重い'],
    crit: ['……あ、あ', '壊れる。俺が、壊れる'],
    revive: ['また、動いた', '起きるのか'],
  },
...
const TALE_ENEMY = {
  ...
  e7: {
    ...
    ask: '——覚えているのが、そんなに大事か。男は、こちらを一度だけ見た。',
  },
  e8: {
...
const TALE_AFTER = {
  ...
  e8: {
    ...
    lead: [
      ...
      'ひと呼吸で年月が過ぎていく記憶の時間が、ひと呼吸ぶんの時間に戻っていた。',
      '',
...
const HELP = {
  ...
  credit: {
    h: 'この作品について',
    t: '画像は **Midjourney**、音楽と効果音は **Suno** を使って作っています。'
     + '文章やゲームの仕組みは バジルペッパー・オレンジ（https://x.com/Basil_Pepper） が**Claude Code**と一緒に作りました。',
  },
};
...
const ENDING = {
  ...
  lead: [
    '降りやんだように思えた灰は、目に見えない形となってゆっくりと降っていた。',
    '変わったのは速さのほうだった。朝に聞いた花の名前を、夕方まで覚えていられるようになった。それだけのことが、この世界では随分と懐かしい。',
    '半年ほどたって、各地の街の家々に賑わいが戻り始めた。誰かが思い出したのではない。忘れる前に、話をするだけの時間が戻ってきたのだ。',
    '灰の底で何があったのかは、どこにも記録されていない。過ぎてしまった何百年ぶんの記憶の年月も、戻ってはこなかった。記憶から消えた街は消えたままで、忘れた名前は忘れたままだ。',
    '戻ってきたのは、これから先の時間だけだった。あの日あそこで勝ち取れたものは、それでぜんぶで——たぶん、それが世界を繋ぎとめた。',
  ],

  party: 'そのあと世界がどうなったのかは、どこにも書かれていない。ただ、{names}は、まだ一緒にいるのかもしれない。',

  close: [
    'いつか、あなたもこの旅を忘れる。記憶の灰が言ったことは、嘘や脅しなどではなかった。誰も、忘れることからは逃れられない。',
    'それでも——忘れるまでには、まだ、これだけの時間がある。',
  ],
  last: '——忘れるまでは、たしかに覚えている。',
...
const CREDITS = {
  ...
  roll: [
    ...
    { kind: 'sub',  value: 'Suno' },
    { kind: 'space', value: 20 },
    { kind: 'role',  value: '開発・制作補助' },
    { kind: 'sub',  value: 'Claude Code（Opus 5）' },
    { kind: 'space', value: 160 },
    { kind: 'text',  value: 'この物語のなかで、あなたは十二人の登場人物に出会ってくれました。' },
    { kind: 'text',  value: '十二枚の風景と、また八つの宿敵との戦いを乗り越えました。' },
    { kind: 'text',  value: 'そのうちのいくつかは、もう思い出せないかもしれません。' },
    { kind: 'space', value: 120 },
    { kind: 'text',  value: 'それでも、感謝を伝えたいと思います。' },
    { kind: 'text',  value: '忘れるまでのあいだ、たしかに覚えてくれたことを。' },
    { kind: 'space', value: 80 },
  ],

  last: [
    'この物語も、彼らの旅路も、いつかは記憶から消えてしまうでしょう。',
    '忘れたときは、詩を思い出して、また会いに来てください。',
    'ほら、灰の上には、まだ足あとが残っています。',
  ],
  poemHead: 'あなたが繋ぎとめた旅の記憶',
