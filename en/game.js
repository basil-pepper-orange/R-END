...
const SAVE_KEY = 'remains_en_save_v6';
const SAVE_BAK = 'remains_en_save_v6_bak';
...
function loadSave() {
  ...
  if (bak) {
    ...
    console.warn('The save could not be read, so an earlier copy was restored');
    return bak;
  }
  return newSave();
}
...
function save() {
  try {
    ...
    if (!saveFailed) {
      saveFailed = true;
      toast('This browser cannot save your progress (private mode, perhaps)');
    }
  }
}
...
const MEM_ALPHA = '0123456789ABCDEFGHJKMNPQRSTVWXYZ';

const MEM_BITS = 5;
const MEM_MAX_SHARDS = 32767;

const MEM_KANA_JA = 'あいうえおかきくけこさしすせそたちつてとなにのはひふへほまみむめもやゆよらりるれんがぎぐげござじずぜぞだでどばびぶべぼぱぴぷぺぽ';
...
BitIn.prototype.get = function (n) {
  let v = 0;
  for (let k = 0; k < n; k++) {
    if (this.i >= this.bits.length) throw new Error('That watchword looks too short');
...
function encodeMemory(s) {
  ...
  return bitsToWord(o.bits, MEM_ALPHA, MEM_BITS);
}
...
function wordToBits(word, alpha, width) {
  ...
  for (const ch of word) {
    ...
    if (v < 0) throw new Error(`“${ch}” is not one of the memory letters`);
    for (let k = width - 1; k >= 0; k--) bits.push((v >> k) & 1);
  }
  return bits;
}

function encodeMemoryJa(s) {
  const word = encodeMemory(s);
  return bitsToWord(wordToBits(word, MEM_ALPHA, MEM_BITS), MEM_KANA_JA, MEM_BITS_JA);
}
...
function decodeMemory(text) {
  ...
  if (!cands.length) {
    ...
    kanaFromPoem(text);
    throw new Error('No watchword was entered');
  }
  let last = null;
  for (const cand of cands) {
    try {
      ...
      lastMemoryWasJa = !!cand.ja;
      return s;
    } catch (e) { last = e; }
  }
  throw last || new Error('The watchword could not be read');
}

let lastMemoryWasJa = false;

function decodeMemoryFrom(cand) {
  const c = (cand && cand.word !== undefined) ? cand
          : { word: cand, alpha: MEM_ALPHA, width: MEM_BITS, ja: false };
  if (!c.word) throw new Error('No watchword was entered');
  const bits = wordToBits(c.word, c.alpha, c.width);
  const r = new BitIn(bits);
  const fmt = r.get(4);
  if (fmt < 1 || fmt > MEM_FMT_MAX) throw new Error('That watchword is not in the right form');
  ...
  if (nc > CHARACTERS_ALL.length || nl > LANDSCAPES.length || ne > ENEMIES_ALL.length) {
    throw new Error('That watchword is from a newer version of the game');
  }
  const s = {
  ...
  if (r.get(12) !== memChecksum(payload)) throw new Error('The watchword does not spell out correctly');
  while (r.i < bits.length) { if (r.get(1)) throw new Error('The watchword does not spell out correctly'); }

  if (!Object.keys(s.chars).length) throw new Error('That memory has no companions in it');
...
function poemFromKana(kana) {
  ...
  let cur = [];
  [...kana].forEach((ch, i) => {
    const v = MEM_ALPHA.indexOf(ch);
    cur.push(T[i % 3][v]);
    if (i % 3 === 2) { lines.push(cur.join(' ')); cur = []; }
  });
...
function kanaFromPoem(text) {
  ...
  while (i < s.length) {
    ...
    for (let v = 0; v < tbl.length; v++) {
      ...
      const o = old[v] && String(old[v]).toLowerCase();
      if (o && o.length > len && s.startsWith(o, i)) { hit = v; len = o.length; }
    }
    if (hit < 0) throw new Error(`Cannot read the part around “${s.slice(i, i + 12)}…”`);
    out += MEM_ALPHA[hit];
    i += len;
    pos++;
  }
...
function memCandidates(text) {
  const list = [];
  const push = (word, alpha, width, ja) => {
    if (!word) return;
    if (list.some((c) => c.word === word && c.alpha === alpha)) return;
    list.push({ word: word, alpha: alpha, width: width, ja: !!ja });
  };

  if (looksJapanese(text)) {
    const kj = memNormalizeJa(text);
    if (kj && [...kj].every((ch) => MEM_KANA_JA.indexOf(ch) >= 0)) push(kj, MEM_KANA_JA, MEM_BITS_JA, true);
    try { push(kanaFromPoemJa(text), MEM_KANA_JA, MEM_BITS_JA, true); }
    catch (e) {  }
    return list;
  }

  const k = memNormalize(text);
  if (k && [...k].every((ch) => MEM_ALPHA.indexOf(ch) >= 0)) push(k, MEM_ALPHA, MEM_BITS, false);
  try { push(kanaFromPoem(text), MEM_ALPHA, MEM_BITS, false); }
  catch (e) {  }
  return list;
}

function kanaFromPoemJa(text) {
  const T = [JA_POEM_A, JA_POEM_B, JA_POEM_C];
  let s = String(text || '');
  try { s = s.normalize('NFC'); } catch (e) {}
  s = s.replace(/[\s　、。，．・…「」『』（）()\-—_|/／]/g, '');

  const OLD = (typeof JA_POEM_OLD !== 'undefined' && JA_POEM_OLD) || [{}, {}, {}];
  let out = '', i = 0, pos = 0;
  while (i < s.length) {
    const tbl = T[pos % 3];
    const old = OLD[pos % 3] || {};
    ...
    if (hit < 0) throw new Error(`Cannot read the Japanese watchword around “${s.slice(i, i + 5)}…”`);
    out += MEM_KANA_JA[hit];
    i += len;
    pos++;
  }
...
function poemFromKanaJa(kana) {
  const T = [JA_POEM_A, JA_POEM_B, JA_POEM_C];
  const lines = [];
  let cur = '';
  [...kana].forEach((ch, i) => {
    const v = MEM_KANA_JA.indexOf(ch);
    cur += T[i % 3][v];
...
function saveBlobName() {
  ...
  return `remains_en_${d.getFullYear()}${p(d.getMonth() + 1)}${p(d.getDate())}_${p(d.getHours())}${p(d.getMinutes())}.json`;
}
...
function exportSave() {
  ...
  toast('A backup file has been written');
}
...
function copySave() {
  ...
  if (navigator.clipboard) {
    navigator.clipboard.writeText(json)
      .then(() => toast('Copied. Paste it somewhere safe, such as a notes app'))
      .catch(() => prompt('Copy all of this text and keep it somewhere safe', json));
  } else {
    prompt('Copy all of this text and keep it somewhere safe', json);
  }
}
...
function importSave(text) {
  ...
  try {
    ...
    toast('That data is not in the right shape');
    return false;
  }
  if (!s || s.v !== 6 || !s.chars) { toast('That does not look like data from this game'); return false; }
  const n = Object.keys(s.chars).length;
  if (!confirm(`Loading this will overwrite your current journey.\n(${n} companions / ${s.shards} shards)\nContinue?`)) return false;
  S = s;
  save();
  toast('Data loaded');
...
function awakenMemory(text) {
  let s;
  lastMemoryWasJa = false;
  try {
    s = decodeMemory(text);
  } catch (e) {
    toast(e.message || 'The watchword could not be read');
    return false;
  }
  const n = Object.keys(s.chars).length;
  ...
  const fromJa = lastMemoryWasJa ? '\n\nThis was written in the Japanese version.' : '';
  if (!confirm(`Waking this memory will erase your current journey and replace it.\n\n${n} companions / ${s.shards} shards / ${cl} defeated${rc ? '\nMemorial: the most recent entry' : ''}${fromJa}\n\nContinue?`)) return false;

  s.playerName = S && S.playerName ? S.playerName : '';
  (s.records || []).forEach((r) => { if (!r.n) r.n = (s.playerName || '').slice(0, 12); });
  S = s;
  save();
  toast('The memory has been woken');
...
function renderMemView() {
  ...
  $('mem-note').innerHTML = poem
    ? `A verse of ${poemFromKana(kana).split('\n').length} lines. Copy the whole of it, and you can take this journey up again on any device.`
    : `${kana.length} letters in all. Write it down, and you can take this journey up again on any device.`;
  $('mem-copy').textContent = poem ? 'Copy this verse' : 'Copy the watchword';
  const tg = $('mem-toggle');
  if (tg) tg.textContent = poem ? 'Show as letters' : 'Show as a verse';
}

function copyMemory() {
  let t;
  try { t = memViewText(encodeMemory(S)); } catch (e) { toast('The watchword could not be made'); return; }
  const done = memView === 'poem' ? 'The verse has been copied' : 'The watchword has been copied';
  if (navigator.clipboard) {
    navigator.clipboard.writeText(t)
      .then(() => toast(done))
      .catch(() => prompt('Please write this down by hand', t));
  } else {
    prompt('Please write this down by hand', t);
  }
}

function copyMemoryJa() {
  let t;
  try { t = poemFromKanaJa(encodeMemoryJa(S)); } catch (e) { toast('The watchword could not be made'); return; }
  if (navigator.clipboard) {
    navigator.clipboard.writeText(t)
      .then(() => toast('The Japanese verse has been copied'))
      .catch(() => prompt('Please write this down by hand', t));
  } else {
    prompt('Please write this down by hand', t);
  }
...
function openStorageModal(focus) {
  ...
  const p = (n) => String(n).padStart(2, '0');
  const when = at ? `${at.getFullYear()}.${p(at.getMonth() + 1)}.${p(at.getDate())} ${p(at.getHours())}:${p(at.getMinutes())}` : 'Nothing has been saved yet';
  let ok = true;
  try { encodeMemory(S); } catch (e) { ok = false; }

  openModal(`
    <div class="md__title">The Memory of a Journey</div>
    <div class="md__sub">Turn this journey into a verse and carry it with you</div>

    <div class="memo__head">
      <span class="md__label">Set it down</span>
    </div>
    <div class="memo memo--poem" id="mem-box">${ok ? '' : '<span class="memo__err">The watchword could not be made</span>'}</div>
    <div class="memo__note" id="mem-note"></div>
    <div class="md__row">
      <button type="button" class="btn btn--sub" id="mem-copy">Copy this verse</button>
    </div>

    <div class="md__label">Wake a memory</div>
    <textarea class="memo__in" id="mem-in" rows="4" spellcheck="false" autocapitalize="off" autocomplete="off" placeholder="Write the verse you kept, here"></textarea>
    <div class="md__row">
      <button type="button" class="btn btn--sub btn--awaken" id="mem-load">Wake a memory</button>
    </div>
    <div class="memo__note">Copy in the verse you kept, exactly as it is.<br>Extra spaces, line breaks and punctuation are fine.<br>Waking it will overwrite the journey you are playing now.<br><b>A verse from the Japanese version works here too</b> — paste it in as it is.</div>

    <details class="memo__more">
      <summary>Carry this journey to the Japanese version</summary>
      <div class="md__desc">
        The Japanese version keeps its verses in Japanese, so it cannot read the verse above.<br>
        Here is <b>the same journey, written the way the Japanese version reads it</b>.
      </div>
      <div class="memo memo--poem memo--ja" id="mem-box-ja">${ok ? '' : '<span class="memo__err">The watchword could not be made</span>'}</div>
      <div class="md__row">
        <button type="button" class="btn btn--sub" id="mem-copy-ja">Copy the Japanese verse</button>
      </div>
    </details>

    <details class="memo__more">
      <summary>Where the data of this world lives</summary>
      <div class="md__desc">
        Your progress is saved automatically <b>inside the browser you are using</b>.<br>
        Last saved: <b>${when}</b>.<br>
        It survives closing the tab and restarting the computer.<br><br>
        It is <b>lost</b> in the cases below. When that happens, wake it from your watchword.
        <ul class="md__ul">
          <li>when you clear site data with the browser’s “clear browsing data”</li>
          <li>when you play in a private or incognito window</li>
          <li>when you open it in another browser or on another device</li>
          <li>on iPhone Safari, when you <b>do not open it for 7 days</b></li>
        </ul>
        A watchword does <b>not</b> carry <b>leftover experience, the name you carved, or your list of records</b>.<br>
        To keep everything, write it out to a file below.
      </div>
      <div class="md__row">
        <button type="button" class="btn btn--sub" id="md-export">Write to a file</button>
        <button type="button" class="btn btn--sub" id="md-import-file">Restore from a file</button>
      </div>
  ...
  if (ok) {
    try { $('mem-box-ja').textContent = poemFromKanaJa(encodeMemoryJa(S)); }
    catch (e) { $('mem-box-ja').textContent = ''; }
  }
  $('mem-copy-ja').onclick = copyMemoryJa;
...
const Sound = {
  ...
  init() {
    try {
      this.on = localStorage.getItem('remains_en_mute') !== '1';
      const b = localStorage.getItem('remains_en_bgmlv');
      const f = localStorage.getItem('remains_en_sfxlv');
  ...
  playBgm(name) {
    ...
    const want = CONFIG.bgm[use] ? this.absUrl(CONFIG.bgmDir + CONFIG.bgm[use]) : null;
    const now = this.bgmEl && this.bgmEl.src;
    if (this.bgmEl && !this.bgmEl.paused && want && now && now === want) {
      this.bgmName = name;
      this.bgmSrcName = use;
      return;
    }
    ...
    if (!this.on || this._suspended) {
      if (el.src !== this.absUrl(src)) el.src = src;
      el.volume = 0;
      el.pause();
      return;
    }
  ...
  toggle() {
    this.on = !this.on;
    try { localStorage.setItem('remains_en_mute', this.on ? '0' : '1'); } catch (e) {}
  ...
  setBgmLevel(i) {
    ...
    try { localStorage.setItem('remains_en_bgmlv', String(this.bgmLv)); } catch (e) {}
  ...
  setSfxLevel(i) {
    ...
    try { localStorage.setItem('remains_en_sfxlv', String(this.sfxLv)); } catch (e) {}
    renderSoundPanel();
...
function renderSoundPanel() {
  ...
  p.innerHTML =
    ...
    row('Effects', Sound.sfxLv, CONFIG.sfxLevels.length, 'sfx') +
    `<button type="button" class="snd__mute ${Sound.on ? '' : 'is-off'}" id="snd-mute">
       ${Sound.on ? 'Mute everything' : 'Turn sound on'}
     </button>`;
...
const SCREENS = {
  ...
  gacha:  { el: 'screen-gacha',  title: 'The Calling', back: true,  dock: false },
  party:  { el: 'screen-party',  title: 'Party', back: true,  dock: true  },
  book:   { el: 'screen-book',   title: 'Archive',   back: true,  dock: true  },
  records:{ el: 'screen-records', title: 'Memorial',    back: true,  dock: false },
  stage:  { el: 'screen-stage',  title: 'The Road',         back: true,  dock: false },
  tale:   { el: 'screen-tale',   title: 'Story',         back: true,  dock: false },

  story:  { el: 'screen-story',  title: 'Beginnings',    back: true,  dock: false },

  after:  { el: 'screen-tale',   title: 'Story',         back: false, dock: false },
  battle: { el: 'screen-battle', title: 'Battle',         back: false, dock: false },
  result: { el: 'screen-result', title: 'Result',     back: false, dock: false },

  ending: { el: 'screen-ending', title: 'Journey’s End',   back: false, dock: false },
  ...
  for (let i = ENEMIES.length - 1; i >= 0; i--) {
    ...
    if (S.cleared[e.id] && e.story) {
      return i === ENEMIES.length - 1
        ? e.story + '<br>There are vistas yet unseen, and they may still be met.'
        : e.story;
    }
  }
  return 'A world returned entirely to ash.<br>Only the vistas you have gathered still hold their color.';
}
...
function renderHome() {
  ...
  const hi = document.querySelector('.home__inner');
  if (hi) hi.classList.toggle('home__inner--cleared', endingCleared());
}
...
function openGacha(type) {
  ...
  $('gacha-name').textContent = isChar ? 'The Calling' : 'Lost Vistas';
  $('gacha-desc').textContent = isChar
    ? 'Call back, out of the ash, those who still have a name.'
    : 'Draw up a fragment from when the world was still beautiful.';
...
function updatePullBtn() {
  ...
  $('btn-pull').firstChild.nodeValue = S.shards < cost ? 'Not enough shards' : 'Draw';
  ...
  if (b10) {
    ...
    b10.firstChild.nodeValue = `Draw ×${n}`;
...
function showMultiResult(got, isChar, shardBack) {
  ...
  const cards = got.map(({ result, isNew }) => `
    <div class="gm__card${isNew ? ' is-new' : ''}${isChar ? '' : ' gm__card--land'}">
      ...
      ${isNew ? '<span class="gm__new">new</span>' : ''}
    </div>`).join('');

  const sum = multiSummary(got, isChar);
  const rows = sum.map((m) => {
    const bits = [];
    if (m.isNew) bits.push('<span class="up">Joined you</span>'.replace('Joined you', isChar ? 'Joined you' : 'Remembered'));
    if (m.lvFrom != null) bits.push(`Lv.${m.lvFrom} → <span class="up">Lv.${m.lvTo}</span>`);
    if (m.rkFrom != null) bits.push(`Rank ${m.rkFrom} → <span class="up">${m.rkTo}</span>`);
    if (m.shards > 0) bits.push(`<span class="up">Shards +${comma(m.shards)}</span>`);
    if (!bits.length) bits.push('<span class="dim">No change</span>');
  ...
  host.innerHTML = `
    ...
    ${shardBack > 0 ? `<div class="gm__back">already at their peak — shards <b class="up">+${comma(shardBack)}</b></div>` : ''}
  `;
  host.hidden = false;
...
function showGachaResult(result, isNew, isChar, dupeInfo) {
  ...
  let note;
  if (isNew) {
    note = isChar ? `${result.name} has joined you.` : `You remembered “${result.name}.”`;
  } else if (isChar && dupeInfo && dupeInfo.kind === 'levelup') {
    note = `You met ${result.name} again. <span class="up">Lv.${dupeInfo.before} → Lv.${dupeInfo.after}</span>`
         + (dupeInfo.rAfter > dupeInfo.rBefore
            ? `<br><small>The bond deepened <span class="up">Rank ${dupeInfo.rBefore} → ${dupeInfo.rAfter}</span> (base stats +${Math.round((CONFIG.charRankBonus || 0) * 100)}%)</small>` : '');
    Sound.play('common/victory');
  } else if (isChar && dupeInfo && dupeInfo.kind === 'rankonly') {
    note = `You met ${result.name} again. <span class="up">Rank ${dupeInfo.rBefore} → ${dupeInfo.rAfter}</span>`
         + `<br><small>Base stats rose by +${Math.round((CONFIG.charRankBonus || 0) * 100)}%</small>`;
    Sound.play('common/victory');
  } else if (isChar) {
    note = `${result.name} can go no further. Shards +${CONFIG.dupeCharShards}`;
  } else if (dupeInfo && dupeInfo.kind === 'rankup') {
    note = `The memory of “${result.name}” deepened. <span class="up">Rank ${dupeInfo.before} → ${dupeInfo.after}</span><br>
            <small>The skill can now be used ${dupeInfo.after} times</small>`;
    Sound.play('common/victory');
  } else {
    note = `“${result.name}” can go no further. Shards +${CONFIG.dupeLandShards}`;
  }
...
function renderParty() {
    ...
    if (cid) {
      ...
      div.innerHTML = `
        ...
        ${eq ? `<span class="pslot__eq"><img src="${CONFIG.landDir}${eq.file}" alt=""><i>${landRank(eq.id)}</i></span>`
             : '<span class="pslot__noeq">no skill</span>'}
      ...
      div.onclick = () => toast('Tap a companion below to add them');
    }
  ...
  rw.innerHTML = '';
  {
    ...
    if (cid && S.chars[cid]) {
      ...
      div.innerHTML = `<span class="pslot__empty">+</span><span class="pslot__cap">no reserve</span>`;
      div.onclick = () => toast('Tap a companion below and choose “Make reserve”');
    }
    rw.appendChild(div);
  }
  ...
  $('party-total').innerHTML =
    `<span>Total HP <b>${comma(hp)}</b></span><span>ATK <b>${comma(atk)}</b></span><span>DEF <b>${comma(def)}</b></span>`;
  ...
  CHARACTERS.forEach((c) => {
    ...
    card.onclick = own ? () => openCharModal(c.id) : () => toast('You have not met them yet');
...
function setPicked(info) {
  ...
  toast('Tap where you want them (tap again to cancel)', CONFIG.toastSkillMs);
}
...
function commitSwap(src, dst) {
  ...
  toast(dst.kind === 'list' || dst.kind === 'listarea' ? 'Removed from the party' : 'Swapped');
  return true;
}
...
$('screen-party').addEventListener('click', (ev) => {
  ...
  if (DRAG.picked) {
    ...
    if (same) { toast('Cancelled'); return; }
    commitSwap(src, dst);
  }
}, true);
...
function openCharModal(cid) {
  ...
  openModal(`
    <div class="md__head">
      ...
      <div>
        ...
        <div class="md__sub">${elemBadge(c.elem, 'elem--sm')} ${c.title} ／ Lv.${own.lv}${maxed ? ' (max)' : ''}
          / <b class="crank">Rank ${charRank(cid)}</b> / ${CONFIG.maxCharRank}</div>
        <div class="md__rank">
          ${charRankStars(cid)}
          <small>Base stats <b class="up">+${Math.round((charRankMul(cid) - 1) * 100)}%</b>
          ${charRank(cid) < CONFIG.maxCharRank
            ? `／ drawing them again raises the rank (a Lost Art never lowers it)`
            : '／ it can deepen no further'}</small>
        </div>
    ...
    ${c.skill ? `<div class="md__skill${skillUsesOf(cid) <= 0 ? ' is-locked' : ''}">
      ...
      <small>${c.skill.desc}<br>${skillUsesOf(cid) > 0
        ? `Usable up to <b class="up">${skillUsesOf(cid)}</b> times per battle.`
        : 'Equip a Vista to make this usable.'}</small>
    </div>` : ''}

    ${c.lost ? `<div class="md__skill md__skill--lost${canLost(cid) ? '' : ' is-locked'}">
      <b>${c.lost.name}<span class="md__lostmark">Lost Art</span></b>
      <small>${c.lost.desc}<br>${canLost(cid)
        ? `Can be used <b class="lost">once HP falls below half</b>. Using it <b class="lost">costs ${CONFIG.lostLevelCost} levels</b>（Lv.${own.lv} → Lv.${Math.max(1, own.lv - CONFIG.lostLevelCost)}). Usable with no Vista carried.`
        : `Becomes usable at Lv.${CONFIG.lostMinLevel} and above.`}</small>
    </div>` : ''}

    <div class="md__exp">
      <div class="bar"><span style="width:${maxed ? 100 : (own.exp / need * 100)}%"></span></div>
      <small>${maxed ? 'It can grow no further' : `${need - own.exp} EXP to the next level`}</small>
    </div>

    <div class="md__stats">
      <div class="md__stat"><span>HP</span><b>${st.hp}</b>${eq ? `<i>+${eq.bonus.hp}</i>` : ''}</div>
      <div class="md__stat"><span>ATK</span><b>${st.atk}</b>${eq ? `<i>+${eq.bonus.atk}</i>` : ''}</div>
      <div class="md__stat"><span>DEF</span><b>${st.def}</b>${eq ? `<i>+${eq.bonus.def}</i>` : ''}</div>
    </div>

    <div class="md__label">Vista carried</div>
    ${eq ? `<div class="md__eq">
              <img src="${CONFIG.landDir}${eq.file}" alt="">
              <div>
                <b>${eq.name}</b> ${rankStars(landRank(eq.id))}
                <small>HP +${st.eqBonus.hp} / ATK +${st.eqBonus.atk} / DEF +${st.eqBonus.def}<br>
                Skill usable <b class="up">${landRank(eq.id)}</b> times<br>
                ${landEffText(eq, landRank(eq.id))}</small>
              </div>
            </div>`
          : `<div class="md__warn">No Vista is carried, so <b>the skill cannot be used</b>.</div>`}
    <div class="md__row md__row--tight">
      <button type="button" class="btn btn--sub" id="md-equip">
        ${eq ? 'Change Vista' : 'Carry a Vista'}
      </button>
      ${eq ? `<button type="button" class="btn btn--sub" id="md-unequip">Put it down</button>` : ''}
    </div>

    <div class="md__label">Party</div>
    <div class="md__row md__row--tight">
      ...
      <button type="button" class="btn btn--sub" id="md-reserve">
        ${S.reserve === cid ? 'Remove from reserve' : 'Make reserve'}
      </button>
    </div>
  `);
  ...
  if ($('md-unequip')) $('md-unequip').onclick = () => {
    S.chars[cid].eq = null; save(); closeModal(); renderParty(); toast('Vista removed');
  };
  $('md-party').onclick = () => {
    if (inParty) {
      S.party = S.party.filter((x) => x !== cid);
    } else {
      if (S.party.length >= CONFIG.partySize) { toast(`A party holds ${CONFIG.partySize} at most`); return; }
...
function openEquipModal(cid) {
  ...
  if (!owned.length) {
    toast('You have no Vistas yet');
    return;
  }
  openModal(`
    <div class="md__title">Choose a Vista to carry</div>
    <div class="md__sub">One Vista can be carried by one companion only.<br>
      Its rank is how many times their skill can be used (drawing it again raises the rank)</div>
    <div class="md__label">Vistas held ${owned.length} / ${LANDSCAPES.length}
      <span class="dim"> — tap one to see what it does</span></div>
    ...
    peek.innerHTML = `
      <div class="eqpeek__box">
        ...
        <div>
          ...
          <small>HP +${bn.hp} / ATK +${bn.atk} / DEF +${bn.def}<br>
            Skill usable <b class="up">${rk}</b> times<br>
            ${eff || 'no special effect'}</small>
        </div>
      </div>
      ${holder && !mine
        ? `<div class="eqpeek__note"><b>${charById(holder).name}</b> is carrying it right now</div>`
        : ''}
      <button type="button" class="btn eqpeek__go" id="eqpeek-go"${mine ? ' disabled' : ''}>${
        mine ? 'You are carrying this Vista'
             : (holder ? `Take it from ${charById(holder).name}` : 'Carry this Vista')}</button>
    `;
    peek.hidden = false;
  ...
  owned.forEach((l) => {
    ...
    b.innerHTML = `<img src="${CONFIG.landDir}${l.file}" alt="">
      <span class="bcard__rank">${rk}×</span>
      ${holder ? `<span class="bcard__eq">${holder === cid ? 'carried' : 'held'}</span>` : ''}
      <span class="bcard__cap">${l.name}<br>HP+${bn.hp} ATK+${bn.atk} DEF+${bn.def}
...
function renderBook() {
  ...
  LANDSCAPES.forEach((l, i) => {
    ...
    card.innerHTML = has
      ? `<img src="${CONFIG.landDir}${l.file}" alt="">
         ${holder ? `<span class="bcard__eq">held</span>` : ''}
         <span class="bcard__rank">${landRank(l.id)}×</span>
         <span class="bcard__cap">${l.name}<br>${rankStars(landRank(l.id))}</span>`
      : `<span class="bcard__num">${String(i + 1).padStart(2, '0')}</span>`;
    card.onclick = has ? () => openLandModal(l) : () => toast('A vista not yet seen');
    list.appendChild(card);
  });
  ...
  CHARACTERS.forEach((c) => {
    ...
    card.onclick = has
      ...
      : () => toast('You have not met them yet');
  ...
  openModal(`
    <div class="md__head">
      ...
      <div>
        ...
        <div class="md__sub">${rankStars(landRank(l.id))} Rank ${landRank(l.id)} / ${CONFIG.maxLandRank}</div>
        <div class="md__desc">
          HP +${landBonus(l, landRank(l.id)).hp} / ATK +${landBonus(l, landRank(l.id)).atk} / DEF +${landBonus(l, landRank(l.id)).def}<br>
          Whoever carries it can <b class="up">use their skill ${landRank(l.id)} times</b><br>
          ${landEffText(l, landRank(l.id))}<br>
          ${landRank(l.id) < CONFIG.maxLandRank
            ? `<span class="dim">At the next rank: HP +${landBonus(l, landRank(l.id) + 1).hp} / ATK +${landBonus(l, landRank(l.id) + 1).atk} / DEF +${landBonus(l, landRank(l.id) + 1).def}, skill ${landRank(l.id) + 1}×</span>`
            : '<span class="up">It can deepen no further</span>'}<br>
          ${holder ? `carried by ${charById(holder).name}` : 'No one is carrying it'}
        </div>
      </div>
    </div>
    <div class="md__row">
      <button type="button" class="btn btn--sub" id="md-zoom-btn">View larger</button>
    </div>
  `);
  const open = () => openViewer(CONFIG.landDir + l.file, l.name, `${'★'.repeat(l.rarity)}　HP+${l.bonus.hp} ATK+${l.bonus.atk} DEF+${l.bonus.def}`);
...
function ultNote(e) {
  ...
  if (e.forget) {
    ...
    return ` / <b>lost when ${e.forget.cap} years pass</b>${at ? ` / past ${at} years: <b>${e.ult.name}</b>` : ''}`;
  }
  return ` / at 100% Collapse: <b>${e.ult.name}</b>`;
}
...
function renderStages() {
  ...
  if (typeof PROLOGUE !== 'undefined' && PROLOGUE) {
    ...
    pn.textContent = 'about this world';
    pb.appendChild(pn);
  ...
  ENEMIES.forEach((e, i) => {
    ...
    b.innerHTML = `
      ...
      ${cleared ? '<span class="stage__clear">defeated</span>'
                : (!open ? '<span class="stage__lock">not reached</span>' : '')}
      <div class="stage__label">${e.label}</div>
      <div class="stage__name">${open ? `${elemBadge(e.elem, 'elem--sm')} ${e.name}` : '? ? ?'}</div>
      ${!open ? '<div class="stage__desc">Defeat the one before it, and the way opens.</div>' : ''}
      ${open && met ? `<div class="stage__desc">${e.desc}</div>
                <div class="stage__stats">HP ${e.hp} / ATK ${e.atk} / DEF ${e.def}${e.aoeEvery ? ` / strikes the whole party every ${e.aoeEvery} turns` : ''}${ultNote(e)}</div>
                <div class="stage__skills">Moves: ${(e.skills || (e.skill ? [e.skill] : [])).map((k) => k.name).join(', ')}</div>
                <div class="stage__reward">Reward: EXP ${rewardExp(e)} / shards ${rewardShards(e)}</div>` : ''}
    `;
    if (open) {
      const nw = document.createElement('span');
      nw.className = 'stage__tale' + (taleSeen(e.id) ? '' : ' is-new');
      nw.textContent = taleSeen(e.id) ? 'Story' : 'A story not yet read';
      b.appendChild(nw);
    }
    b.onclick = () => {
      if (!S.party.length) { toast('There is no one in your party'); return; }
      openTale(e.id);
    };
    wrap.appendChild(b);
  });
...
function buildTale(eid) {
  ...
  const voices = party.map((cid) => {
    ...
    return {
      ...
      act: v ? v.act : `${c.name} said nothing, and looked at the vista.`,
      line: v ? v.line : '...Let us go',
    };
  });
  ...
  const reserve = rid ? { name: charById(rid).name, line: rv ? rv.line : '...Go on ahead' } : null;
...
function renderTaleBody(d) {
  ...
  const say = (name, line, cls) =>
    `<div class="tale__say ${cls || ''}"><b>${escapeHtml(name)}</b><span>&ldquo;${escapeHtml(line)}&rdquo;</span></div>`;

  $('tale-body').innerHTML = `
    ${e.image ? `<div class="tale__art"><img src="${e.image}" alt=""></div>` : ''}
    <div class="tale__head">
      <div class="tale__chapter">${escapeHtml(d.chapter)}${d.isNew ? '<span class="tale__new">first reading</span>' : ''}</div>
    ...
    ${d.reserve ? `<div class="tale__reserve">${d.after ? 'From behind, ' + escapeHtml(d.reserve.name) + ' comes walking up.' : 'Behind them, ' + escapeHtml(d.reserve.name) + ' waits in reserve.'}<span>&ldquo;${escapeHtml(d.reserve.line)}&rdquo;</span></div>` : ''}
...
function openPrologue() {
  ...
  `;
  $('btn-tale-go').textContent = 'Back to the road';
  $('btn-tale-skip').hidden = true;
  go('tale');

  $('hud-title').textContent = 'Prologue';
  $('screen-tale').scrollTop = 0;
}
...
function buildAfterTale(eid, out) {
  ...
  const reserve = rid ? { name: charById(rid).name, line: rv ? rv.line : '...Glad you are unhurt' } : null;
...
function openAfterTale(eid, out) {
  ...
  $('btn-tale-go').textContent = 'See the result';
  $('btn-tale-skip').hidden = true;
  go('after');
  $('screen-tale').scrollTop = 0;
}
...
function renderEnding() {
  ...
  `;
  $('btn-ending-go').textContent = d.btn || 'To the credits';
  $('btn-ending-back').textContent = d.btnBack || 'Return';
...
function renderCredits() {
  ...
  `;
  $('btn-credits-skip').textContent = d.btnSkip || 'Skip to the end';
  $('btn-credits-ok').textContent = d.btnEnd || 'The End';
}
...
function landTurnEffects() {
  ...
  B.units.forEach((u, i) => {
    ...
    if (hp > 0) {
      ...
      if (total > 0) notes.push(`<span class="eff">${effName('healParty')}</span> whole party <span class="heal">+${total}</span>`);
    }
    const rc = effOf(u, 'recall');
    if (rc > 0 && u.skillMax > 0 && u.skillLeft < u.skillMax && Math.random() < rc) {
      u.skillLeft++;
      notes.push(`<span class="eff">${effName('recall')}</span> ${u.name} regained one skill use`);
    }
  });
...
function bringReserve() {
  ...
  pushLog('<b>— this is not over yet</b>');
...
function swapReserve(i, forced) {
  ...
  if (B.reserve.hp <= 0) { toast(`${B.reserve.name} has fallen too`); return false; }
  const u = B.units[i];
  if (!forced && (!u || u.hp > 0)) { toast('Only a fallen companion can be swapped here'); return false; }
  ...
  applyGuardOthers();
  Sound.play('common/heal');
  pushLog(`<b>${nu.name}</b> swapped in for ${gone} and stepped forward`);
  renderBattle();
  ...
  toast(`${nu.name} has joined the line`);
  return true;
}
...
function startBattle(eid) {
  ...
  pushLog(`<b>${e.name}</b> stands in the way`);
  ...
  if (e.forget) {
    ...
    if (f.note || f.noteHow) {
      pushLog(`<span class="ult">Years</span>　${escapeHtml(fill(f.note))}　`
...
function renderBattle() {
  ...
  $('turn-label').textContent = `Turn ${B.turn}`;
  ...
  if (B.pinch) {
    ...
    foe.push(`<b class="boost">Rising</b>${up(`skill +${Math.round(((g.power || 1) - 1) * 100)}%`)}`);
  }
  if (rage > 0) foe.push(up(`ATK +${Math.round(rage * (CONFIG.rageAtkMax || 0))}%`));

  if (fg) foe.push(dn(`+${yearsPerTurn(B.turn)} yrs this turn`));
  if (B.eAtkUp > 0) foe.push(up(`ATK +${Math.round(B.eAtkUpRate * 100)}% (${B.eAtkUp})`));
  if (B.defDown > 0) foe.push(dn(`DEF −${Math.round(B.defDownRate * 100)}% (${B.defDown})`));
  if (B.atkDown > 0) foe.push(dn(`ATK −${Math.round(B.atkDownRate * 100)}% (${B.atkDown})`));
  if (B.eSilence > 0) foe.push(dn(`skills sealed (${B.eSilence})`));
  if (B.burn > 0) foe.push(dn(`burning (${B.burn})`));

  atkUpList().forEach((x) => ally.push(up(`ATK +${Math.round(x.rate * 100)}% (${x.turns})`)));
  if (B.critUp > 0) ally.push(up(`crit ×${B.critUpMul} (${B.critUp})`));
  if (B.pAtkDown > 0) ally.push(dn(`ATK −${Math.round(B.pAtkDownRate * 100)}% (${B.pAtkDown})`));

  const mkRow = (who, list) => list.length
    ? `<div class="mkrow"><b class="mkrow__who">${who}</b>${list.join('　')}</div>`
    : '';

  $('enemy-rage').innerHTML = mkRow('Foe', foe) + mkRow('You', ally);
  ...
  B.units.forEach((u, i) => {
    ...
    div.innerHTML = `
      <div class="unit__face">
        ...
        ${u.seal > 0 ? `<span class="unit__seal">held ${u.seal}</span>` : ''}
        ${u.shield > 0 ? `<span class="unit__shield">◇ ${u.shield}</span>` : ''}
        ${u.charge ? '<span class="unit__mark">charge</span>' : ''}
        ${u.chant ? '<span class="unit__mark unit__mark--chant">weave</span>' : ''}
      ...
      ${dead && B.reserve && B.reserve.hp > 0 ? `<button type="button" class="unit__swap" data-a="swap">Swap in ${B.reserve.name}</button>` : `
      <div class="unit__cmds">
        <button type="button" class="unit__cmd ${u.action === 'attack' ? 'is-on' : ''}" data-a="attack" ${dead || u.seal > 0 ? 'disabled' : ''}>Strike</button>
        ${own
      ...
      <button type="button" class="unit__skill ${u.action === 'skill' ? 'is-on' : ''} ${u.skillMax <= 0 ? 'is-locked' : ''}" data-a="skill"
        ...
        ${u.skillMax <= 0 ? 'no Vista' : (sk ? sk.name : 'no skill')}<small>${'●'.repeat(Math.max(0, u.skillLeft))}${'○'.repeat(Math.max(0, u.skillMax - u.skillLeft))}</small>
      </button>
...
`;
  ...
  $('btn-exec').textContent = B.pickRevive != null ? 'Cancel' : 'Go';
  renderSwapBtn();
}
...
function renderSwapBtn() {
  ...
  b.textContent = B && B.swapPick ? 'Cancel' : 'Swap';
}

function swapLive(i) {
  if (!B || B.busy || B.over || !B.reserve) return false;
  const u = B.units[i];
  if (!u || u.hp <= 0) { toast('Fallen companions are swapped from the card itself'); return false; }
  if (u.seal > 0) { toast(`${u.name} is pinned down and cannot be swapped`); return false; }
  if (B.reserve.seal > 0) { toast(`${B.reserve.name} still cannot move`); return false; }

  if (B.reserve.hp <= 0 && !B.units.some((t, k) => k !== i && t.hp > 0)) {
    toast('That would leave no one able to stand at the front'); return false;
  }
  ...
  applyGuardOthers();
  Sound.play('common/heal');
  pushLog(`<b>${nu.name}</b> swapped in for ${u.name} and stepped forward`);
  renderBattle();
  if (nu.hp > 0) speakUnit(i, 'join', true);
  flushPops();
  const card = document.querySelectorAll('#units .unit')[i];
  if (card) { card.classList.remove('is-swapin'); void card.offsetWidth; card.classList.add('is-swapin'); }
  toast(`${nu.name} and ${u.name} changed places`);
  return true;
}
...
$('units').addEventListener('click', (ev) => {
  ...
  if (B.pickRevive != null) {
    ...
    if (!B.units[k] || B.units[k].hp > 0) { toast('Choose a fallen companion'); return; }
    const who = B.units[B.pickRevive];
    if (who) who.revivePick = k;
    B.pickRevive = null;
    toast(`Wake ${B.units[k].name}`);
    renderBattle();
    execute();
    return;
  }
  ...
  if (B.swapPick) {
    ...
    else { B.swapPick = false; toast('That companion cannot be swapped'); renderBattle(); }
    return;
  }
  ...
  if (a === 'lost') {
    ...
    if (!lost) return;
    if (!canLost(u.cid)) {
      toast(`${lost.name} needs Lv.${CONFIG.lostMinLevel || 6} or above`);
      return;
    }
    if (!lostReady(u)) {
      toast(`${lost.name} can only be used below half HP`);
      return;
    }
  ...
  if (a === 'skill') {
    const u = B.units[i];
    const sk = charById(u.cid).skill;
    if (u.skillMax <= 0) { toast('Carry a Vista to unlock this one’s skill'); return; }
...
function unitDescOf(btn) {
  ...
  if (a === 'lost')  return c.lost
    ? `${c.lost.name} — ${c.lost.desc} (costs Lv−${CONFIG.lostLevelCost || 5})` : '';
  return '';
}
...
$('btn-swap').addEventListener('click', () => {
  ...
  if (B.pickRevive != null) { toast('Choose who to wake first'); return; }
  if (B.swapPick) { B.swapPick = false; renderBattle(); return; }
  if (B.reserve.seal > 0) { toast(`${B.reserve.name} still cannot move`); return; }
  B.swapPick = true;
  renderBattle();
  toast(B.reserve.hp > 0
    ? `Tap the companion to swap with ${B.reserve.name}`
    : `Tap the companion to swap with ${B.reserve.name} (fallen)`);
});
...
function recordCard(rec, mine) {
  ...
  const feat = (m) => {
    ...
    const acts = [];
    if (aAtk)   acts.push(`<span>Strike <i>${aAtk}</i></span>`);
    if (aOwn)   acts.push(`<span>${c.own ? c.own.name : 'Signature'} <i>${aOwn}</i></span>`);
    if (aSkill) acts.push(`<span>${c.skill ? c.skill.name : 'Skill'} <i>${aSkill}</i></span>`);
    if (aLost)  acts.push(`<span class="is-lost">Lost Art <i>${aLost}</i></span>`);
    return `<div class="rec__st">
        <span class="rec__dmg">dealt <b>${comma(dmg || 0)}</b></span>
        <span class="rec__taken">taken <b>${comma(taken || 0)}</b></span>
  ...
  return `<div class="rec">
    <div class="rec__top">
      <span class="rec__name">${escapeHtml(rec.n || 'Nameless')}</span>
      <span class="rec__date">${fmtDate(rec.d)}</span>
    </div>
    <div class="rec__enemy">
      ${e && e.image ? `<img src="${e.image}" alt="">` : ''}
      <div>
        defeated <b>${e ? e.name : '? ? ?'}</b>
        <small>${rec.t} turns${total ? ` / ${comma(total)} damage dealt` : ''}</small>
        ${lost.length ? `<small class="rec__lost">used ${lost.join(', ')}</small>` : ''}
      </div>
    </div>

    <div class="rec__slots">${slots}</div>
    ${st ? `<div class="party__total rec__total">
      <span>Total HP <b>${comma(st[0])}</b></span><span>ATK <b>${comma(st[1])}</b></span><span>DEF <b>${comma(st[2])}</b></span>
    </div>` : ''}
...
function renderRecords() {
  ...
  wrap.innerHTML = list.length
    ...
    : '<p class="stone__empty">Nothing has been carved here yet.<br>After a victory, choose “Carve this battle” and it will remain here.</p>';
...
function showSharedRecord() {
  ...
  setTimeout(() => {
    openModal(`
      <div class="md__title">Someone’s memory</div>
      <div class="md__sub">This inscription reached you</div>
      <div class="stone__list stone__list--one">${recordCard(rec, false)}</div>
      <div class="md__row"><button type="button" class="btn btn--sub" id="md-rec-close">I will set out too</button></div>
    `);
...
function makeShareCard(rec) {
  return new Promise((resolve) => {
    ...
    Promise.all(srcs.map((x) => (x ? load(x) : Promise.resolve(null)))).then((imgs) => {
      ...
      g.fillText(`Defeated ${e ? e.name : '? ? ?'}`, 62, 168);

      g.fillStyle = '#96a0b1'; g.font = `400 28px ${sans}`;
      g.fillText(`${rec.n || 'Nameless'}　/　${rec.t} turns　/　${fmtDate(rec.d).split(' ')[0]}`, 64, 214);
      ...
      if (lands.length) {
        ...
        g.fillText(`Vistas carried: ${lands.join(', ')}`, 62, y0 + cardH + 52);
      }
      const lost = (rec.l || []).map((cid) => charById(cid) && charById(cid).lost.name).filter(Boolean);
      if (lost.length) {
        g.fillStyle = '#ff9db4'; g.font = `400 24px ${sans}`;
        g.fillText(`Lost Arts used: ${lost.join(', ')}`, 62, y0 + cardH + 88);
      }
...
async function shareImage(rec) {
  toast('Making the image of the inscription…');
  const cv = await makeShareCard(rec);
  const blob = await new Promise((res) => cv.toBlob(res, 'image/png'));
  if (!blob) { toast('The image could not be made'); return; }
  const file = new File([blob], 'remains.png', { type: 'image/png' });
  ...
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `remains_${rec.e}_${rec.d}.png`;
  a.click();
  setTimeout(() => URL.revokeObjectURL(a.href), 4000);
  toast('Image saved');
}
...
function copyRecordUrl(rec) {
  ...
  if (navigator.clipboard) {
    navigator.clipboard.writeText(u).then(() => toast('Link copied'), () => prompt('You can share this link', u));
  } else {
    prompt('You can share this link', u);
  }
}

function askPlayerName(then) {
  const cur = S.playerName || '';
  const v = prompt('Choose the name to carve on the Memorial (up to 12 letters)', cur || '');
  if (v === null) return;
...
function doLeave(rec, btn) {
  ...
  saveRecord(rec);
  if (btn) { btn.disabled = true; btn.textContent = 'The memory has been kept'; }
  Sound.play('common/victory');
  toast('The battle has been carved');
  setTimeout(() => { renderRecords(); go('records'); }, 500);
}

function playerName() {
  return (S.playerName || '').trim() || 'Nameless';
}
...
function payLostCost(u, i) {
  ...
  pushLog(`<b>${u.name}</b> let a memory go <span class="lost">Lv.${own.lv + cost} → Lv.${own.lv}</span>`);
}
...
function strikeEnemy(u, power, readable, k) {
  ...
  const tag = (mul > 1 ? '　<span class="weak">Weakness!</span>' : '')
            + (crit ? '　<span class="crit">Critical!</span>' : '')
            + (rm < 1 ? '　<span class="seal">Read and turned aside</span>' : '');
...
function nowAt(rate, kept, word) {
  return kept > rate ? ` (now <span class="dn">${word} −${Math.round(kept * 100)}%</span>)` : '';
}

function atkUpLog(rate, total) {
  const one = `Every ally’s attack rose by ${Math.round(rate * 100)}%`;
  return atkUpList().length > 1
    ? `${one} (<span class="up">total ATK +${Math.round(total * 100)}%</span>)`
    : one;
}
...
function addYears(n, label) {
  ...
  pushLog(`<span class="ult">${label}　${n} more years slipped by</span>　<b>${B.years} yrs</b>`);
  popEnemy(`+${n} yrs`, 'seal');
...
function cutYears(kind, sk) {
  ...
  const why = extra > 0
    ? `<span class="eff">(${escapeHtml(sk.name || '')} wound back a further ${extra} years)</span>` : '';
  pushLog(`<span class="eff">${got} years were wound back</span>${why}　<b>${B.years} yrs</b>`);
  popEnemy(`−${got} yrs`, 'heal');
}
...
function loseByYears() {
  if (!B || B.over) return;
  B.yearsOver = true;
  pushLog(`<span class="ult">${forgetOf().cap} years have passed. ...Even the fighting has been forgotten</span>`);
  endBattle(false);
}
...
function playCutIn({ img, name, from, lines, foe, lost, hold }, done) {
  ...
  $('skillcut-name').innerHTML = from
    ? `<span class="skillcut__from">${escapeHtml(from)}’s Lost Art</span>${escapeHtml(name)}`
    : escapeHtml(name);
...
function boostTag() {
  return (B && B.pinch) ? ' <span class="boost">Rising</span>' : '';
}
...
function playHalfBreak(done) {
  ...
  pushLog(`<b>${e.name}</b>　<span class="ult">${cut.name || 'It begins to come apart'}</span>`);
  ...
  playCutIn({ img: e.image || '', name: cut.name || 'It begins to come apart',
...
function halfBreakExtra(cut) {
  ...
  const fg = forgetOf();
  if (fg && fg.pinchMul > 1) {
    pushLog(`<span class="ult">The years now run ${fg.pinchMul}× faster</span>`
          + `　<span class="eff">+${yearsPerTurn(B.turn)} yrs this turn</span>`);
    toast(`From here the years run ${fg.pinchMul}× faster`, 3600);
  }
  ...
  if (n > 0) {
    ...
    pushLog(took > 0
      ? `<span class="seal">${took} skill uses were taken</span>　(all allies −${n})`
      : '<span class="seal">There were no skill uses left to take</span>');
  }
...
function playHalfGrant(cut, done) {
  ...
  if (cut.healParty) {
    ...
    pushLog(`<span class="heal">Even the wounds are made never to have been</span>`
          + `${all.length ? `　(${all.join(', ')} fully restored)` : ''}`
          + `${woke.length ? `　<span class="eff">${woke.join(', ')} rose again</span>` : ''}`);
  }
  ...
  if (cut.refillSkill) {
    ...
    pushLog(names.length
      ? `<span class="eff">Even the skills spent are made never to have been</span>　(${names.join(', ')} recovered theirs)`
      : '<span class="eff">Even the skills spent are made never to have been</span>');
  }
  ...
  if (freeing) {
    ...
    pushLog(`<span class="lost">The Lost Arts are unbound</span>`
          + `　<span class="eff">usable at any HP</span>${names.length ? `　(${names.join(', ')})` : ''}`);
    toast('HP and skills fully restored, and the Lost Arts are unbound', 4200);
...
function playHalfHeal(cut, done) {
  ...
  const heal = () => {
    ...
    if (got > 0) {
      popEnemy(`+${got}`, 'heal');
      pushLog(`<b>${B.e.name}</b> recovered <span class="heal">${got}</span>`);
    }
    renderBattle();
    flushPops();
    setTimeout(() => {
...
function raiseMax(to, done) {
  ...
  pushLog(`<span class="ult">The vessel of ${B.e.name} itself is growing deeper</span>`);
  Sound.play('common/enemy_skill', 'common/enemy_attack');
  ...
  const tick = () => {
    ...
    setTimeout(() => {
      clear();
      pushLog(`<b>${B.e.name}</b>　<span class="ult">max HP ${to}</span>`);
      done();
    }, Math.max(ms, 340));
  };
  tick();
...
function useLost(u, i) {
  ...
  if (sk.gamble > 0 && Math.random() >= sk.gamble) {
    screenFlash('lost');
    pushLog('<span class="seal">...A miss. Nothing happens</span>');
    toast(`${sk.name} missed`);
    payLostCost(u, i);
    return;
  }
  ...
  if (sk.power > 0) {
    ...
    shakeScreen(true);
    let line = n > 1 ? `${n} hits　<span class="dmg">${total}</span>${tag}`
                     : `<span class="dmg">${total}</span> damage${tag}`;
    if (sk.drain > 0) {
      const got = healUnit(u, i, Math.round(total * sk.drain));
      line += `　recovered <span class="heal">+${got}</span>`;
    }
    pushLog(line);
  }
  ...
  if (sk.healAll > 0) {
    ...
    if (back.length) pushLog(`<b>${back.join(' and ')}</b> rose again`);
    if (total > 0) pushLog(`The party recovered <span class="heal">+${total}</span>`);
  ...
  if (sk.healSelf > 0) {
    ...
    if (got > 0) pushLog(`<b>${u.name}</b> recovered <span class="heal">+${got}</span>`);
  }
  ...
  if (sk.healReserve > 0 && B.reserve) {
    ...
    if (was <= 0) {
      r.action = 'attack';
      pushLog(`<b>${r.name}</b>, in reserve, <span class="heal">drew breath again</span>`);
    } else if (r.hp > was) {
      pushLog(`<b>${r.name}</b>, in reserve, recovered <span class="heal">+${r.hp - was}</span>`);
    }
  }

  if (sk.cut > 0) {
    B.guardAll = sk.cut;
    pushLog(sk.cut >= 1 ? 'This turn, nothing reaches the party at all'
                        : `The party braced (${Math.round(sk.cut * 100)}% less damage this turn)`);
  }
  if (sk.up > 0) {
  ...
  if (sk.down > 0) {
    ...
    pushLog(`${B.e.name}’s guard opened wide (DEF −${Math.round(sk.down * 100)}%)`
      + nowAt(sk.down, kept, 'DEF'));
  }
  if (sk.atkDown > 0) {
    const kept = keepStronger('atkDown', 'atkDownRate', sk.atkDown, sk.turns || 3);
    pushLog(`${B.e.name}’s strength was pared away (ATK −${Math.round(sk.atkDown * 100)}%)`
      + nowAt(sk.atkDown, kept, 'ATK'));
  }
  if (sk.back > 0) {
    B.rageBack += sk.back;

    if (!forgetOf()) {
      pushLog(rageNow() <= 0 ? 'The Collapse was stilled completely' : `The Collapse was wound back ${sk.back} turns`);
    }
  }
  if (sk.silence > 0) {
    B.eSilence = Math.max(B.eSilence || 0, sk.silence);
    pushLog(`<span class="seal">${B.e.name} cannot recall its moves (${B.eSilence} turns)</span>`);
  }

  if (sk.burn && sk.burn.turns > 0) {
    B.burn = Math.max(B.burn || 0, sk.burn.turns);
    B.burnRate = Math.max(B.burnRate || 0, sk.burn.rate || 0);
    pushLog(`<span class="eff">${B.e.name} goes on burning</span> (${Math.round(B.burnRate * 100)}% of max HP each turn / ${B.burn} turns)`);
  }
  ...
  if (sk.critUp && sk.critUp.turns > 0) {
    ...
    pushLog(`Every ally’s critical rate became <span class="up">${B.critUpMul}×</span> (${B.critUp} turns)`);
  }

  if (sk.selfSeal > 0) {
    u.seal = (sk.selfSeal | 0) + 1;
    pushLog(`<b>${u.name}</b> has spent his power (<span class="seal">cannot move for ${sk.selfSeal} turns</span>)`);
  }
  if (sk.refillAll > 0) {
    const names = [];
    B.units.forEach((t) => {
    ...
    if (names.length) pushLog(`${names.join(' and ')} recovered their skills`);
  }
}
...
function enemyBurnTick() {
  ...
  pushLog(`<span class="eff">Ashfire</span>　<span class="dmg">${d}</span> to ${B.e.name} (${B.burn} turns left)`);
  renderBattle();
...
function useOwn(u, i, done) {
  ...
  switch (o.type) {
    ...
    case 'cover':
      ...
      pushLog('Stepped in front of the others');
      break;

    case 'parry':
      B.parry = { i, power: o.power || 2.0, used: false };
      pushLog('Measuring the distance');
      break;
    ...
    case 'charge':
      ...
      if (o.back) {
        B.rageBack += o.back;
        if (!forgetOf()) pushLog(`<span class="eff">The Collapse was wound back ${o.back} turns</span>`);
      }
      pushLog('Gathering for the next blow');
      break;
    ...
    case 'rush': {
      ...
      pushLog(`${n} hits　<span class="dmg">${total}</span>${tag}`);
      break;
    }
    ...
    case 'readAhead': {
      ...
      pushLog(`<span class="eff">Every ally’s attack rose by ${Math.round(rate * 100)}%</span>`
        + (atkUpList().length > 1 ? ` (<span class="up">total ATK +${Math.round(tot * 100)}%</span>)` : ''));
      break;
    }
    ...
    case 'aim': {
      ...
      slashFx('slash');
      pushLog(`<span class="dmg">${r.d}</span>${r.tag}　<span class="eff">its guard broke open by ${Math.round(rate * 100)}%</span>`
        + nowAt(rate, kept, 'DEF'));
      break;
    }
    ...
    case 'chant':
      u.chant = o.power || 1.5;
      pushLog('<span class="eff">Words were woven into the next skill</span>');
      break;
    ...
    case 'record': {
      ...
      slashFx('slash');
      pushLog(`<span class="dmg">${r.d}</span>${r.tag}　<span class="eff">the move he saw is written down</span>`);
      break;
    }
    ...
    case 'bet': {
      ...
      if (Math.random() < rate) {
        ...
        pushLog(`<span class="eff">It lands</span> (${mul.toFixed(1)}×)　<span class="dmg">${r.d}</span>${r.tag}`
              + (u.betRun > 1 ? `　<span class="eff">${u.betRun} in a row</span>` : ''));
      } else {
        u.betRun = 0;
        const lose = Math.max(1, Math.round(u.max * (o.hurt || 0.10)));
        u.hp = Math.max(1, u.hp - lose);
        popUnit(i, `-${lose}`, '');
        pushLog(`<span class="seal">It does not</span>　${u.name} lost <span class="dmg">${lose}</span>`);
      }
      break;
    }
    ...
    case 'salvage': {
      ...
      if (canPick) {
        ...
        const rest = max === Infinity ? '' : `, ${max - u.salvaged} left`;
        pushLog(`<span class="eff">Arrows gathered again (skills +${got}${rest})</span>`);
      } else {
        const r = strikeEnemy(u, o.power || 0.8);
        slashFx('slash');
        pushLog(`<span class="dmg">${r.d}</span>${r.tag}　<span class="eff">there are no arrows left to gather</span>`);
      }
      break;
    }
    ...
    case 'cure': {
      ...
      if (tu.seal > 0) { tu.seal = 0; note += `　<span class="eff">able to move again</span>`; }
      pushLog(note);
      break;
    }
...
function useSkill(u, i) {
  ...
  u.skillLeft = Math.max(0, u.skillLeft - 1);
  if (u.chant) { pushLog('<span class="eff">The woven words carry through</span>'); u.chant = 0; }
  ...
  switch (sk.type) {
    case 'strike': {
      ...
      slashFx('heavy', 'sky');
      pushLog(`<span class="dmg">${r.d}</span> damage${r.tag}`);
      break;
    }
    case 'multi': {
      ...
      for (let k = 0; k < n; k++) {
        ...
        weak = r.tag.replace(/　<span class="crit">Critical!<\/span>/, '');
        slashFxAt(k, 'heavy', 'sky');
      }
      const critTag = crits > 0 ? `　<span class="crit">${crits} critical!</span>` : '';
      pushLog(`${n} hits　<span class="dmg">${total}</span>${weak}${critTag}`);
      break;
    }
    case 'silence': {
      const r = strikeEnemy(u, sk.power);
      ...
      pushLog(`<span class="dmg">${r.d}</span>${r.tag}　<span class="seal">${B.e.name} cannot recall its moves (${B.eSilence} turns)</span>`);
      break;
    }
    case 'drain': {
      const r = strikeEnemy(u, sk.power);
      ...
      pushLog(`Took <span class="dmg">${r.d}</span> and recovered <span class="heal">+${got}</span>${r.tag}`);
      break;
    }
    case 'break': {
      const r = strikeEnemy(u, sk.power);
      ...
      pushLog(`<span class="dmg">${r.d}</span>　${B.e.name}’s guard loosened${r.tag}`
        + nowAt(sk.down, kept, 'DEF'));
      break;
    }
    case 'foresee': {
      ...
      pushLog(`<span class="dmg">${r.d}</span>`
            + (forgetOf() ? '' : `　the Collapse was wound back ${sk.back} turns`) + r.tag);
      break;
    }
    case 'weaken': {
      const r = strikeEnemy(u, sk.power);
      ...
      pushLog(`<span class="dmg">${r.d}</span>　${B.e.name}’s strength was pared away (ATK −${Math.round(sk.down * 100)}%)${r.tag}`
        + nowAt(sk.down, kept, 'ATK'));
      break;
    }
    case 'refill': {
      const names = [];
      ...
      pushLog(names.length
        ? `${names.join(' and ')} remembered their skills <span class="sk">+${sk.amount || 1}</span>`
        : 'No one has forgotten a skill yet');
      break;
    }
    case 'healAll': {
      let total = 0;
      ...
      pushLog(`The whole party recovered <span class="heal">+${total}</span>`);
      break;
    }
    case 'healOne': {
      ...
      if (target) {
        ...
        pushLog(got > 0 ? `${target.name} recovered <span class="heal">+${got}</span>`
                        : `${target.name} has not a scratch`);
      }
      break;
    }
    case 'guardAll': {
      B.guardAll = sk.cut;
      pushLog(`The party braced (${Math.round(sk.cut * 100)}% less damage this turn)`);
      break;
    }
    case 'buffAtk': {
    ...
    case 'revive': {
      ...
      if (target) {
        ...
        pushLog(`<b>${target.name}</b> rose again`);
        enemySay('revive');
        ...
        pushLog(total > 0 ? `No one had fallen. The whole party recovered <span class="heal">+${total}</span>`
                          : 'No one has fallen. There is no need for this now');
      }
      break;
    }
    default:
      pushLog('But nothing happened');
  }
...
function landRevive(u, i) {
  ...
  popUnit(i, `+${u.hp}`, 'heal');
  Sound.play('common/heal');
  pushLog(`<span class="eff">${effName('revive')}</span>　<b>${u.name}</b> rose again`);
  enemySay('revive');
  return true;
}
...
function reviveFallen(hpRate) {
  ...
  pushLog(`<b>${u.name}</b>, in reserve, <span class="heal">drew breath again</span>`);
  toast(`${u.name} drew breath again in reserve`);
  enemySay('revive');
  return true;
}
...
function execute() {
  ...
  if (ask >= 0) {
    B.pickRevive = ask;
    B.swapPick = false;
    toast(`${B.units[ask].name}: choose who to wake`);
    renderBattle();
    return;
  }
  B.pickRevive = null;
  ...
  const next = () => {
    ...
    if (s.kind === 'unit') {
      ...
      if (u.seal > 0) {
        pushLog(`<b>${u.name}</b> cannot move (<span class="seal">${u.seal} turns left</span>)`);
        renderBattle();
      ...
      pushLog(`<b>${u.name}</b> strikes${mul > 1 ? '<span class="eff"> (the gathered blow)</span>' : ''}　<span class="dmg">${r.d}</span>${r.tag}`);
      withPursuit();
      return;
    ...
    function enemyStrike(esk, isUlt) {
      ...
      const hitOne = (t, rate, single, k) => {
        ...
        if (single && B.parry && !B.parry.used && B.parry.i === t.i && t.u.hp > 0) {
          B.parry.used = true;
          popUnit(t.i, 'read', 'heal');
          pushLog(`<b>${t.u.name}</b> read the attack through`);
          const r = strikeEnemy(t.u, B.parry.power);
          slashFx('heavy');
          pushLog(`Answering cut　<span class="dmg">${r.d}</span>${r.tag}`);
          return 0;
        }
        ...
        if (single && B.cover && B.cover.i !== t.i) {
          ...
          if (cu && cu.hp > 0) {
            pushLog(`<b>${cu.name}</b> covered ${t.u.name}`);
            t = { u: cu, i: B.cover.i };
            covered = true;
          }
        }
      ...
      const wtag = (t) => !t.u ? ''
        : (t._weak ? '<span class="weak">Weakness!</span>' : '')
        + (t._crit ? '<span class="crit">Critical!</span>' : '');
      ...
      if (isUlt) {
        ...
        if (esk.kind === 'aoe') {
          ...
          pushLog(`<span class="dmg">${hitOne(target, esk.power, true)}</span> to ${target.u.name}${wtag(target)}`);
        }
        ...
        if (enemyAlive && esk.seal) {
          ...
          if (t) {
            ...
            if (g > 0 && Math.random() < Math.min(1, g)) {
              ...
              pushLog(`<b>${t.u.name}</b> was not pinned down`
                    + `（<span class="eff">${effName('sealGuard')}</span>）`);
            } else {

              t.u.seal = esk.seal.turns + 1;
              t.u.action = 'attack';
              popUnit(t.i, 'held', 'seal');
              pushLog(`<b>${t.u.name}</b> was pinned down (<span class="seal">cannot act for ${esk.seal.turns} turns</span>)`);
            }
          }
        }
        ...
        if (enemyAlive && esk.selfHeal) {
          ...
          if (B.ehp > before) {
            ...
            pushLog(`${B.e.name} recovered <span class="heal">${B.ehp - before}</span>`);
          }
        }
        ...
        if (esk.atkDown) {
          ...
          pushLog(`The party’s strength dulled (ATK −${Math.round(esk.atkDown.rate * 100)}%)`);
        }

        if (esk.selfAtkUp) {
          B.eAtkUp = esk.selfAtkUp.turns + 1;
          B.eAtkUpRate = esk.selfAtkUp.rate;
          pushLog(`<b>${B.e.name}</b> grew stronger (ATK +${Math.round(esk.selfAtkUp.rate * 100)}%)`);
        }
        ...
        if (esk.rageUp) {
          ...
          pushLog(`<span class="ult">The Collapse advanced ${esk.rageUp} turns</span>`);
        }
        ...
        if (enemyAlive && esk.twice) {
          ...
          if (alive2.length) {
            ...
            pushLog(`<b>${B.e.name}</b> moved again　<span class="dmg">${hitOne(t2, 1, true, 1)}</span> to ${t2.u.name}${wtag(t2)}`);
          }
        }
      } else {
        ...
        if (isAoe) {
          ...
          pushLog(`<b>${B.e.name}</b> strikes the whole party${boostTag()}`);
          ...
          for (let k = 0; k < times; k++) {
            ...
            const head = times > 1
              ? (k === 0 ? `<b>${B.e.name}</b> strikes` : `<b>${B.e.name}</b> struck again`)
              : `<b>${B.e.name}</b> strikes`;
            pushLog(`${head}　<span class="dmg">${hitOne(t, 1, true, k)}</span> to ${t.u.name}${wtag(t)}`);
          }
        }
      }
      ...
      if (B.pendingThorns > 0) {
        ...
        pushLog(`<span class="eff">${effName('thorns')}</span>　<span class="dmg">${B.pendingThorns}</span> to ${B.e.name}`);
        B.pendingThorns = 0;
      }
...
function fleeBattle() {
  if (!B || B.over) return;
  if (B.busy) { toast('You cannot flee mid-action'); return; }
  if (!confirm('You will flee this battle.\nNo shards and no experience will be gained. Continue?')) return;
  ...
  pushLog('<b>Fled</b>　nothing was gained');
  toast('Fled the battle');
...
function endBattle(win) {
  ...
  if (win) {
    ...
    pushLog(`<b>${B.e.name}</b> faded away`);
...
function showResult(win) {
  ...
  gainers.forEach((cid) => {
    ...
    if (own.lv > before) ups.push(`${charById(cid).name}: Lv.${before} → <span class="up">Lv.${own.lv}</span>`);
  });

  save();

  $('result-h').textContent = win ? 'Victory' : 'Defeat';
  $('result-h').className = 'result__h ' + (win ? 'win' : 'lose');
  $('result-body').innerHTML = `
    ${win ? '' : (B.yearsOver
      ? '<div>— Even the fighting was forgotten. And still, footprints remain on the ash.</div>'
      : '<div>They fell back, outmatched. Something was still gained.</div>')}
    <div>EXP <b>${exp}</b></div>
    <div>Shards <b>+${shards}</b></div>
    ${bonus ? `<div class="up">First-victory bonus: shards +${bonus}</div>` : ''}
    ${ups.length ? '<div>' + ups.join('<br>') + '</div>' : ''}
    ${firstClear && ENEMIES[ENEMIES.length - 1].id !== e.id ? '<div class="up">A new road has opened</div>' : ''}
    ${firstClear && ENEMIES[ENEMIES.length - 1].id === e.id
      ? `<div class="up">${e.forget ? '— Until it is forgotten, it is remembered.' : 'The world remembered.'}</div>` : ''}
  `;
  ...
  rb.hidden = !win;
  rb.disabled = false;
  rb.textContent = 'Carve this battle';
  if (win) {
...
function openHistory() {
  if (!B || !B.history.length) { toast('There are no records yet'); return; }
  let html = `<div class="md__title">Battle log</div>
  ...
  B.history.forEach((h) => {
    if (h.turn !== lastTurn) {
      lastTurn = h.turn;
      html += `<div class="hist__turn">Turn ${h.turn}</div>`;
    }
...
function openModal(html) {
  $('modal-panel').innerHTML =
    '<button type="button" class="md__close" id="md-close" aria-label="Close">×</button>' + html;
  $('modal').hidden = false;
...
$('btn-reset').onclick = () => {
  ...
  if (!confirm(`All ${got} companions and ${ownedLands().length} vistas, every level and everything you have reached, will be gone.\nReally start over?`)) return;
...
document.addEventListener('pointerdown', (e) => {
  ...
  if (CONFIG.helpOnFirst && typeof HELP !== 'undefined') {
    let seen = null;
    try { seen = localStorage.getItem('remains_en_help_seen'); } catch (e) {}
    if (!seen) {
      try { localStorage.setItem('remains_en_help_seen', '1'); } catch (e) {}
      setTimeout(openHelp, 700);
    }
  }
  if (restoredFromBak) {
    setTimeout(() => toast('The save could not be read, so the previous one was restored'), 900);
  }
}
